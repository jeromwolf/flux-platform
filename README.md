...
| Workflow Engine | ~95% | DAG executor, 6 내장 노드, Redis 큐 워커, 스케줄러, 웹훅, SSRF 방어 |
...
| Step | 문서 | 내용 |
|------|------|------|
| Step 1 | `docs/ontology/step1_purpose_definition.md` | 목적 정의, CQ1-CQ5, 도메인 범위, 6대 조인 키 |
| Step 2 | `docs/ontology/step2_reuse_evaluation.md` | 재사용 분석 (S-100, AIS, MAKG 등 10종 판정) |
| Step 3 | `docs/ontology/step3_key_term_enumeration.md` | 핵심 용어 열거 (P0/P1/P2 3계층, 108 관계) |
| Step 4-7 | 준비 중 | 클래스 계층, 속성, 제약조건, 인스턴스 |

## KRISO 1차년도 — 프로젝트 워크벤치

KRISO 1차년도 연구용역(2026, 지식그래프 모델 설계 연구) 착수에 따라 팀 협업용 프로젝트 워크벤치를 운영합니다.

- **워크벤치:** https://imsp-ontology-docs.vercel.app
- **구성:** 프로젝트 대시보드, 15개 요구사항 태스크(REQ·DES·TER·COM), 착수보고서, KRISO 협조요청서, 제출 문서 관리, 회의록, 조사 참고자료
- **REQ-001 산출물 draft 5종:** 데이터 제공처별 현황 조사서, AIS 표준 분석 템플릿, 멀티모달 데이터 인벤토리, 연계 규칙·식별자 체계, 우선순위 선정표
- **소스:** `docs/ontology/` (HTML 워크벤치), `docs/*_draft.md` (산출물 원본)
...
```bash
# 전체 테스트 (4,590+ passed, 90 skipped)
...
├── tests/         # 테스트 (4,590+ unit/harness + 35 Playwright)
├── docs/          # 전략서, 온톨로지 설계
└── scripts/       # 유틸리티 스크립트
```
