...
18. KRISO 1차년도 착수: `docs/ontology/` 프로젝트 워크벤치 구축 — 대시보드, 15개 요구사항 태스크 페이지(REQ·DES·TER·COM), 착수보고서, KRISO 협조요청서, 제출 문서 관리, 회의록, 조사 참고자료. 착수보고서·협조요청서 draft 작성. REQ-001 산출물 5종 draft (데이터 제공처별 현황조사서·AIS 표준분석템플릿·멀티모달 인벤토리·연계규칙/식별자체계·우선순위 선정표). Vercel 배포: imsp-ontology-docs.vercel.app (워크벤치 루트 = index.html). 워크벤치 배포는 `docs/ontology/`에서 `vercel --prod` 실행
